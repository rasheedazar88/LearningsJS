...
function trigger() {
    ...
    for (var i in cdl) {
        obj = cdl[i];
        if (obj.ecommerce && obj.ecommerce.viewcart) {
            var products = obj.ecommerce.viewcart;
            var properties = {};
            properties["productProperties"] = genProductProps(products);
            _cl.pageview("Cart viewed", properties);
            hasEvent = true;
        }
    };
    if (!hasEvent) {
